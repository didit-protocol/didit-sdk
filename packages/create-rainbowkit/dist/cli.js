...
async function run() {
  try {
    ...
    log();
    log(chalk.green(`\u{1F308} Welcome to RainbowKit!`));
    ...
    if (!isValidProjectName(projectPath)) {
      throw new FriendlyError([
        chalk.red(`\u{1F440} The project name you provided is not a valid package name.`),
        `\u{1F64F} ${invalidProjectNameErrorMessage}`
      ].join("\n"));
    }
    if (reservedPackageNames.includes(projectPath)) {
      throw new FriendlyError([
        chalk.red(`\u{1F440} The project name you provided is a reserved package name.`),
        `\u{1F64F} Please use a project name other than "${reservedPackageNames.find((x) => x === projectPath)}".`
      ].join("\n"));
    }
    ...
    log(chalk.cyan(`\u{1F680} Creating a new RainbowKit app in ${chalk.bold(targetPath)}`));
    ...
    log(chalk.cyan(`\u{1F4E6} Installing dependencies with ${chalk.bold(packageManager)}. This could take a while.`));
    ...
    log(chalk.green(`\u{1F308} Done! Thanks for using RainbowKit \u{1F64F}`));
    log();
    log(chalk.cyan(`\u{1F449} To get started, run ${chalk.bold(`cd ${projectPath}`)} and then ${chalk.bold(`${packageManager}${packageManager === "npm" ? " run" : ""} dev`)}`));
    log();
  } catch (err) {
